...
for d = 1:length(analysisDays)
    ...
    for g = 1:2
        ...
        if isempty(grp), continue; end
        x = d + (g-1.5)*barW*2;
        scatter(repmat(x, height(grp), 1), grp.Clostridioides, 140, ...
            groupColors(g+1,:), 'filled', 'MarkerFaceAlpha', 0.7, ...
            'HandleVisibility', 'off');
...
% Legend
scatter(nan, nan, 140, groupColors(2,:), 'filled', 'DisplayName', 'ST1.75');
scatter(nan, nan, 140, groupColors(3,:), 'filled', 'DisplayName', 'ST1.12');
