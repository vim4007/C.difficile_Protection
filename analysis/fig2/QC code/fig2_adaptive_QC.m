%% fig2_adaptive_QC.m
% Quality control checks for adaptive immune flow cytometry data
% Run AFTER fig2_adaptive_analysis.m — requires variables from that script
...
fprintf('\n=== Adaptive Panel Gating Quality Control ===\n');
...
for i = 1:length(mice)
    ...
    mouseGated = sum(sum(mouseCells{:, 17:22}, 2) > 0);
...
if ~warned
    fprintf('  No warnings — all populations have >= 30 cells per mouse\n');
end
