%% Read the raw tables
% Data files required (place in data/raw/mouse/):
%   - Virulence_screen_clean_table.csv
%   - ProtectionScreen_CDI_mouse.csv
vir_tbl = readtable('Virulence_screen_clean_table.csv');
tbl = readtable('ProtectionScreen_CDI_mouse.csv');
tbl = [tbl;vir_tbl];
...
scatter(combined_score.Virulence_Estimate, combined_score.Protection_Estimate, ...
    100, 'filled', 'MarkerFaceAlpha', 0.7);
hold on;
...
text(combined_score.Virulence_Estimate - 1.3, combined_score.Protection_Estimate - 1.3, ...
    combined_score.Strains, 'FontSize', 18, 'Horiz', 'left', 'Vert', 'bottom');

xlabel('Virulence Score'); ylabel('Protection Score');
set(gca,'FontSize',20);
...
% Plot error bars first (so they're behind the points)
hold on;
for i = 1:height(combined_score)
    % Virulence CI as horizontal error bars
    ...
    % Protection CI as vertical error bars
    ...
    % Plot both horizontal (virulence) and vertical (protection) error bars
...
% Plot scatter points on top - no color specified to get default blue
scatter(combined_score.Virulence_Estimate, combined_score.Protection_Estimate, ...
...
% Convert exp_id to string for consistent handling
strain_data.exp_id = string(strain_data.exp_id);

% Identify mice that died or had relweight = 0
...
for i = 1:numel(all_exp_ids)
    ...
    % Check if this mouse died or had relweight = 0
...
% Plot mean with shaded SD
valid = ~isnan(mean_rel);
x = days(valid); 
y = mean_rel(valid); 
e = sd_rel(valid);
e(isnan(e)) = 0; % Handle single experiment case
...
% Calculate error bar values (distance from estimate to CI bounds)
...
% Set x-axis labels to strain names
...
for i = 1:length(strains_of_interest)
    ...
    % Get one observation per exp_id (which already uniquely identifies each mouse)
    ...
    % Print info
    fprintf('Strain %s: %d mice, %d deaths\n', strain, length(final_time), sum(final_death));
    % If all censored, manually create survival curve at 1.0
    if sum(final_death) == 0
        ...
        % FIX: Extend curve to last observation if needed
    ...
    % Plot with different line style
...
%title('Kaplan-Meier Survival Curves by C. diff Strain', 'FontSize', 14);
set(gca,'FontSize',25);
