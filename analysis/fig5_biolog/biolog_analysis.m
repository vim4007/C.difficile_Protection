%% --- Load data ---
B   = readtable('Biolog_growth_matrix.xlsx','VariableNamingRule','preserve');
G   = readtable('strain_groups.xlsx',           'VariableNamingRule','preserve');
MoA = readtable('moas.xlsx',                    'VariableNamingRule','preserve');
...
M     = table2array(B(:,st1));           % 95 metabolites x 21 strains
...
% Green -> blue colormap (low protection = green, high = blue)
...
fprintf('Wilcoxon: %d testable metabolites\n', m_tests);
fprintf('Bonferroni threshold (0.05/%d) = %.2e\n', m_tests, bonf_thr);
...
fprintf('\nMoA Bonferroni threshold (0.05/%d) = %.4f\n', m_cat, bonf_thr_cat);
fprintf('Categories passing Bonferroni: %d\n', sum(MR.sig_bonf));
...
%% --- FIG 1: Waterfall top 10 + bottom 10 metabolites ---
%% --- FIG 1: Waterfall top 10 + bottom 10, vertical bars ---
% W_rho = sortrows(W(~isnan(W.rho),:),'rho','descend');
% combo = [W_rho(1:10,:); W_rho(end-9:end,:)];
% 
% figure('Position',[100 100 1000 600]);
% for i = 1:height(combo)
%     c = cat_map(combo.category{i});
%     bar(i, combo.rho(i),'FaceColor',c,'EdgeColor','k'); hold on;
% 
%     if combo.p_wilcox(i) < bonf_thr
%         mk = '**';
%     elseif combo.p_wilcox(i) < 0.05
%         mk = '*';
%     else
%         mk = '';
%     end
%     if ~isempty(mk)
%         y_off = 0.03 * sign(combo.rho(i));
%         va = 'bottom'; if combo.rho(i)<0, va='top'; end
%         text(i, combo.rho(i)+y_off, mk, 'FontSize',13,'FontWeight','bold', ...
%              'HorizontalAlignment','center','VerticalAlignment',va);
%     end
% end
% 
% set(gca,'XTick',1:height(combo), ...
%     'XTickLabel',arrayfun(@(i) sprintf('%s (n=%d)', combo.Metabolite{i}, combo.n_growers(i)), ...
%                           1:height(combo),'UniformOutput',false), ...
%     'XTickLabelRotation',60);
% yline(0,'k'); xline(10.5,'--','Color',[.6 .6 .6]);
% ylabel('Spearman \rho  (growth vs protection)');
% title('Top 10 / bottom 10 metabolites');
% ylim([-1 1]);
% 
% cats_in_plot = unique(combo.category);
% h = arrayfun(@(j) patch(NaN,NaN,cat_map(cats_in_plot{j})), 1:numel(cats_in_plot));
% legend(h, cats_in_plot,'Location','northeast','Box','off');
% box off;

%% --- FIG 2: Waterfall MoA categories ---
%% --- FIG 2: Waterfall MoA categories (vertical bars) ---
...
% Count metabolites per category (self-contained)
...
for j = 1:numel(cats)
    ...
    % Total metabolites in this category, above the bar
    text(j, 1.03, sprintf('%d', cat_counts(j)), ...
        'HorizontalAlignment','center','FontSize',11,'FontWeight','bold');
end

% X-tick labels show "category (n)" like Fig 2
...
colormap(cmap_gb);
scatter(scores(:,1),scores(:,2),150,prot,'filled','MarkerEdgeColor','k'); hold on;
for i = 1:n_st1
    text(scores(i,1)+0.1, scores(i,2)+0.08, strrep(st1{i},'ST1_','ST1-'),'FontSize',15);
end
if any(strcmp(B.Properties.VariableNames,'VPI'))
    vpi = (double(table2array(B(:,'VPI'))') - mean(X,1)) * coeff;
    scatter(vpi(1),vpi(2),200,'r^','filled','MarkerEdgeColor','k');
    text(vpi(1)+0.15, vpi(2), 'VPI','Color','r','FontWeight','bold','FontSize',15);
end
...
% Build the MoA count matrix: rows = strains, cols = categories
...
% Sort strains by protection, high -> low
...
% RIGHT y-axis: protection line plot + linear fit
yyaxis right
x_vals = (1:n_st1)';
...
% Spearman correlation: niche breadth vs protection
...
xlim([0.5 n_st1+0.5]);
title('Metabolic niche composition by chemical class');

% Legend with category names + total metabolites per class (as before)
cat_counts = arrayfun(@(j) sum(strcmp(cats_per_row, cats{j})), 1:numel(cats));
leg_labels = arrayfun(@(j) sprintf('%s (%d)', cats{j}, cat_counts(j)), ...
                      1:numel(cats),'UniformOutput',false);
legend(b, leg_labels,'Location','northeastoutside','Box','off','FontSize',9);
...
X = M';   % 21 strains x 95 metabolites
...
for i = 1:n_st1
    ...
    % Accumulate importance from full model
